    ...
    public void validate(Object obj, Errors errors) {
        ...
        if (usuari.getNombre() != null && !usuari.getNombre().isEmpty()) {
            ...
            if (!Character.isUpperCase(first)) {
                errors.rejectValue("nombre", "invalid", "El nombre debe comenzar con mayúscula");
            }
        }

        if (usuari.getApellidos() == null || usuari.getApellidos().isEmpty()) {
            errors.rejectValue("apellidos", "required", "El apellidos es obligatorio");
        }

        if (usuari.getApellidos() != null && !usuari.getApellidos().isEmpty()) {
            char first = usuari.getApellidos().charAt(0);
            if (!Character.isUpperCase(first)) {
                errors.rejectValue("apellidos", "inválido", "El apellidos debe comenzar con mayúscula");
            }
        }
        ...
        if (usuari.getPassword()== null|| usuari.getPassword().trim().isEmpty()) {
            ...
            if (password.length()<8) {
                errors.rejectValue("password", "invalid", "Minimo 8 caracters");
            }

            if (!password.matches(".*[A-Z.]*")) {
                errors.rejectValue("password", "invalid", "Debe contener una mayúscula");
            }

            if (!password.matches(".*\\d.*")) {
                errors.rejectValue("password", "invalid", "Debe contener un número");
            }

            if (!password.matches(".*[!@#$%^&*()].*")) {
                errors.rejectValue("password", "invalid", "Debe contener un carácter especial");
            }
        }

        if (usuari.getTelefono() != null && !usuari.getTelefono().matches("\\d+")) {
            errors.rejectValue("telefono", "invalid", "El teléfono solo debe contener números");
        }

        if (!usuari.isConsentimientoRgpd()) {
            errors.rejectValue("consentimientoRgpd", "required", "Debe aceptar el RGPD");
        }
    }
}